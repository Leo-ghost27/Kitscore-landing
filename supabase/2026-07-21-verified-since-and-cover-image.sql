-- Repo-sync, not a new change: applied live on 2026-07-21 (schema_migrations
-- versions 20260721051948 and 20260721110123), verified byte-identical
-- against the live fn_get_evekit_profile definition before writing this
-- file. Found missing from git during a full drift-reconciliation pass --
-- both changes are actively referenced across the Verified Media Kit
-- (evekit.html, evidence.html) with no matching commit anywhere.
--
-- 1. verified_since -- a simple, honest trust-tenure signal on the
--    public Media Kit. Deliberately NOT "reconnected N times"
--    (considered and dropped: a reconnect count doesn't cleanly map to
--    trustworthiness, and would've required moving platform disconnects
--    from hard-delete to a soft-delete/audit-log model just to support
--    a metric of dubious value).
--
--    verified_since = earliest connected_at among CURRENTLY connected
--    OAuth-verified platforms. If a creator disconnects and reconnects,
--    this date correctly moves forward -- it means "continuously
--    verified since," not "first ever verified," which is the honest
--    claim given connects/disconnects aren't otherwise logged. Null if
--    the creator has no OAuth-verified platform right now.
--
-- 2. cover_image_url -- custom cover/banner image for the Verified
--    Media Kit header, behind the avatar/name. Same storage bucket and
--    upload pattern as avatar_url/gallery_images (creator-media bucket,
--    same size/type validation). No separate GRANT needed: the
--    creators table's UPDATE grant to authenticated already covers all
--    columns including future ones added via ALTER TABLE.

ALTER TABLE public.creators ADD COLUMN IF NOT EXISTS cover_image_url text;
