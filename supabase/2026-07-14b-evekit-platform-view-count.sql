-- fn_get_evekit_profile's platforms jsonb already carries follower_count
-- and video_count, but never view_count -- even though the column has
-- existed on platform_connections since 2026-07-09 and the owner-mode
-- RPC (fn_creator_platform_summary) already returns it. This just brings
-- the public/downloadable EveKit's platforms payload in line with what
-- the owner already sees, so subscriber/follower + video + view counts
-- show consistently everywhere (on-screen owner view, public EveKit page,
