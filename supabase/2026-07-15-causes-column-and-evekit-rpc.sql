-- Charity/cause section, per the 4-creator EveKit audit: "genuinely
-- helps brand trust (especially ESG-conscious sponsors), reasonably
-- cheap to add as a text list." Same shape as the existing
-- available_for column (creators.causes text[]).
--
-- Applied directly to production via the Supabase connector; this file
-- tracks it. No separate GRANT needed -- verified beforehand that
-- creators' table-level GRANT UPDATE to authenticated already covers
-- 21/21 existing columns (i.e. it's not a column-restricted grant), so
-- it automatically extends to a new column added via ALTER TABLE. That
-- is NOT true for a brand-new TABLE (see
-- 2026-07-14-grant-collaborations-press-mentions.sql and
-- 2026-07-14-audience-demographics-breakdown.sql, both of which needed
-- an explicit GRANT after being created).

ALTER TABLE public.creators ADD COLUMN causes text[] NOT NULL DEFAULT '{}';
...
RETURNS TABLE(
  ...
  avatar_url text, gallery_images jsonb, available_for text[], causes text[],
...
AS $function$
  SELECT
    ...
    c.avatar_url, c.gallery_images, c.available_for, c.causes,
