// Handler for POST /api/youtube-oauth-callback?action=start
// Authenticated creator only. Returns a Google authorize URL for the
...
// the callback handler (youtube-oauth-callback.js) can later confirm this
// exact request initiated the flow and recover which creator it belongs
// to -- Google's redirect back carries no Authorization header either.
// Same shape as lib/handlers/tiktok-oauth-start.js.
const { adminClient, getAuthedCreator } = require('../supabase-admin');
const { buildAuthorizeUrl, generateState } = require('../google-oauth');

module.exports = async function handleYoutubeOauthStart(req, res) {
  ...
  try {
    ...
    if (!redirectUri) {
      return res.status(500).json({ error: 'YOUTUBE_REDIRECT_URI is not configured' });
    }
