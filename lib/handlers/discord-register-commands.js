...
const { registerGuildSlashCommands, registerGlobalSlashCommands } = require('../discord');
...
module.exports = async function handleDiscordRegisterCommands(req, res) {
  ...
  try {
    ...
    const [guildRegistered, globalRegistered] = await Promise.all([
      registerGuildSlashCommands({ applicationId, guildId, botToken, commands: COMMANDS }),
      registerGlobalSlashCommands({ applicationId, botToken, commands: COMMANDS }),
    ]);
    ...
    return res.status(200).send(
      page(
        ...
        `Registered globally: ${globalRegistered.map((c) => '/' + c.name).join(', ')} ` +
          '(this is what Discovery checks -- can take up to ~1 hour to fully propagate to clients, ' +
          'but the Portal\u2019s checklist should update sooner since it reads the registration itself). ' +
          `Also registered instantly on guild ${guildId} for immediate testing: ` +
