...
// sponsor instead of releasing it to the creator. Only valid BEFORE the
// creator has submitted a deliverable -- at that point nothing's been
// delivered yet, so a unilateral refund is fine. Once
// deliverable_submitted_at is set, this is blocked and the sponsor is
// pointed at escrow-dispute.js instead, which freezes the funds and
// brings Kitscore in rather than letting either side act unilaterally
// on completed work. Only valid while escrow_status is still 'held' --
// once released, funds have already left the platform's balance and
// this endpoint can't touch them.
...
module.exports = async function handleRefund(req, res) {
  ...
  try {
    ...
    if (contract.deliverable_submitted_at) {
      return res.status(400).json({ error: 'The creator has already submitted the deliverable -- use "Report a problem" instead so Kitscore can review it before any money moves.' });
    }
