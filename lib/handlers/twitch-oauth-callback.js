...
const { exchangeCodeForToken, fetchTwitchOwnUser, fetchTwitchFollowerCount, fetchTwitchSubscriberCount, fetchTwitchVideos, computeContentConsistency } = require('../twitch');
...
module.exports = async function handleTwitchOauthCallback(req, res) {
  ...
  try {
    ...
    // Best-effort, non-blocking -- same reasoning as the YouTube/Instagram
    // equivalents. VOD presence, not true stream frequency -- see the
    // limitation noted on fetchTwitchVideos itself. Rebalances the
    // content_consistency family the same way, since this can now
    // coexist with another platform's content_consistency_<platform> row.
    try {
      const videos = await fetchTwitchVideos(tokenData.access_token, user.id);
      const consistencyValue = computeContentConsistency(videos);
      if (consistencyValue !== null) {
        await admin.from('score_components').upsert(
          {
            creator_id: stateRow.creator_id,
            component_key: 'content_consistency_twitch',
            label: 'Content consistency (Twitch)',
            weight: 0.20,
      ...
      console.error('Twitch content_consistency computation failed (non-fatal):', ccErr.message);
    }
