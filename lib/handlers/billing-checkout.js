...
const PRODUCT_CONFIG = {
  ...
  //
  // trialDays: 14 -- Manager gets one free trial (see trial_used guard
  // below); Agency is a paid, in-app upgrade from an already-active
  // Manager account, so it doesn't get a second free trial.
  manager:           { role: 'manager', mode: 'subscription', returnPath: () => 'agency.html', trialDays: 14 },
  agency:            { role: 'manager', mode: 'subscription', returnPath: () => 'profile-manager.html' },
};
...
module.exports = async function handleBillingCheckout(req, res) {
  ...
  try {
    ...
    // Trial eligibility: only the Manager product offers one, and only
    // once per account -- trial_used flips true on first checkout
    // completion (see stripe-webhook.js), so cancel-and-resubscribe
    // lands on a normal paid subscription, not a fresh trial.
    let subscriptionData;
    if (config.trialDays) {
      const admin = adminClient();
      const { data: mgrRow } = await admin.from('managers').select('trial_used').eq('id', buyer.id).maybeSingle();
      if (!mgrRow?.trial_used) {
