  ...
  try {
    ...
    if (product === 'evaluation_unlock') {
      ...
      const { data: sponsorRow } = await admin.from('sponsors')
        .select('plan, evals_used_this_period').eq('id', buyer.id).maybeSingle();
      ...
      if (sponsorRow?.plan === 'starter') {
        ...
        if (used < 25) {
          ...
          await admin.from('sponsors').update({ evals_used_this_period: used + 1 }).eq('id', buyer.id);
