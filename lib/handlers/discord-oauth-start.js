// Handler for POST /api/discord-oauth?action=start
...
const { buildAuthorizeUrl, generateState } = require('../discord');

module.exports = async function handleDiscordOauthStart(req, res) {
  ...
  try {
    ...
    if (!redirectUri) {
      return res.status(500).json({ error: 'DISCORD_REDIRECT_URI is not configured' });
    }
    ...
    return res.status(500).json({ error: err.message || 'Failed to start Discord connection' });
  }
};
