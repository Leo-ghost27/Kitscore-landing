...
module.exports = async function handleCronRepitchNudges(req, res) {
  try {
    ...
    const { data: campaigns, error } = await admin.from('campaigns')
      .select('id, name, creator_id, sponsor_id, completed_at, creator_confirmed, sponsor_confirmed, last_repitch_nudge_sent_at, sponsors!inner(company_name), creators!inner(profiles!inner(email, display_name))')
    ...
    if (!campaigns || !campaigns.length) return res.status(200).json({ ok: true, nudged: 0 });
    ...
    res.status(200).json({ ok: true, eligible: campaigns.length, nudged });
  } catch (err) {
