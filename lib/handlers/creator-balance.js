...
// POST /api/escrow?action=express-dashboard-link
//
// A "Manage in Stripe" shortcut next to the balance box -- Express
// accounts get their own real Stripe dashboard (full payout schedule,
// history, tax forms, bank details) that Kitscore deliberately doesn't
// duplicate. The friction was never the data existing, it was creators
// not knowing that dashboard exists or how to reach it -- it lives at
// a separate login (connect.stripe.com), not something discoverable
// from inside Kitscore on its own.
//
// Login links are single-use and expire a few minutes after creation
// (Stripe's design, not configurable), so this generates one fresh on
// every click rather than ever caching/storing a URL.
async function handleExpressDashboardLink(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const creator = await getAuthedCreator(req);
  if (!creator) return res.status(401).json({ error: 'Not authenticated as a creator' });
  if (!creator.stripe_connect_account_id) {
    return res.status(400).json({ error: 'No connected payout account yet' });
  }

  try {
    const link = await stripe.accounts.createLoginLink(creator.stripe_connect_account_id);
    res.status(200).json({ url: link.url });
  } catch (err) {
    console.error('express dashboard link error:', err);
    res.status(500).json({ error: err.message || 'Could not open Stripe dashboard' });
  }
}

module.exports = { handleBalance, handleExpressDashboardLink };
