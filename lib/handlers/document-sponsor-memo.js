    ...
    if ((safetyAnswers || []).length > 0 || compRiskRows.length > 0) {
      ...
      drawCard(riskHeight, (x, w, cy) => {
        ...
        doc.font('Helvetica').fontSize(9).fillColor(FAINT)
          .text('Family-safe, political, gambling, adult content, and profanity rows are self-reported by the creator, not independently verified. Audience authenticity and content consistency are computed from verified platform data.', x, cy, { width: w });
        cy += 22;
        return cy;
      });
    }
