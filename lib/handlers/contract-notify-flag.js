...
module.exports = async function handleNotifyContractFlag(req, res) {
  ...
  try {
    ...
    // Matches fn_validate_contract_changes exactly (see
    // 2026-08-12-scope-fully-signed-void.sql) -- a fully-signed contract
    // is voidable while this flag is active and no escrow funds have
    // moved. Since this handler only ever runs right after the flag was
    // just set, disputed_at/admin_resolved_at are guaranteed to already
    // be in that "active" state -- the escrow check is what actually
    // varies contract to contract.
    const canVoid = contract.status !== 'void' &&
      (contract.status !== 'fully_signed' || contract.escrow_status === 'not_funded');
