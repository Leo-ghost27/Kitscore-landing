...
module.exports = async function handleAgencyStaffInvite(req, res) {
  ...
  const { data: managerRow } = await admin.from('managers').select('plan').eq('id', owner.id).maybeSingle();
  if ((managerRow?.plan || 'manager') !== 'agency') {
    return res.status(403).json({ error: 'Staff sub-seats are an Agency-plan feature. Upgrade to invite staff.' });
  }
