...
const { deriveVerdict } = require('../ai-brief');
...
module.exports = async function handleEveKit(req, res) {
  ...
  try {
    ...
    // ---------- WHITE-LABEL BRANDING (Manager Seat, opt-in) ----------
    // Same pattern as document-sponsor-memo.js's team branding: only
    // applies when the creator has an ACTIVE manager link (not revoked)
    // AND that manager has actually set a logo/name -- a manager who
    // hasn't set branding just gets the normal Kitscore-branded kit,
    // same "opt-in" behavior as the sponsor-side version.
    let managerBrand = null;
    let managerLogoBuffer = null;
    const { data: activeLink } = await admin.from('manager_creator_links')
      .select('manager_id').eq('creator_id', creator.id).eq('status', 'active').limit(1).maybeSingle();
    if (activeLink) {
      const { data: mgrRow } = await admin.from('managers')
        .select('agency_name, logo_url, plan').eq('id', activeLink.manager_id).maybeSingle();
      // Agency plan only -- white-labeling is an Agency-tier ($149) perk,
      // not included in the base Manager tier ($49). A Manager-plan
      // creator link should never get this for free.
      if (mgrRow && mgrRow.plan === 'agency' && (mgrRow.logo_url || mgrRow.agency_name)) {
        managerBrand = mgrRow;
        if (mgrRow.logo_url) {
          try {
            const imgRes = await fetch(mgrRow.logo_url);
            if (imgRes.ok) managerLogoBuffer = Buffer.from(await imgRes.arrayBuffer());
          } catch (e) {
            console.error('manager white-label logo fetch failed:', e.message);
          }
        }
      }
    }
    ...
    // Eyebrow
    const eyebrowText = managerBrand
      ? `VERIFIED MEDIA KIT · ${(managerBrand.agency_name || 'AGENCY').toUpperCase()}`
      : 'VERIFIED MEDIA KIT · KITSCORE';
    doc.fontSize(8).fillColor(theme.accent).font('Helvetica-Bold')
      .text(eyebrowText, MARGIN, 48, { characterSpacing: 1.5 });
    if (managerLogoBuffer) {
      try {
        doc.image(managerLogoBuffer, W - MARGIN - 72, 40, { fit: [72, 32], align: 'right' });
      } catch (e) {
        console.error('manager white-label logo render failed:', e.message);
      }
    }
    ...
    // ── Score Breakdown ────────────────────────────────────────────────────────
    // NOTE: still a fixed list assuming YouTube for engagement/content-
    // consistency, not dynamic across the multi-platform variants (TikTok/
    // Instagram/Twitch/Discord) app/evekit.html's web page already
    // handles -- pre-existing staleness, out of scope for today's weight
    // fix, flagged here so it isn't mistaken for new.
    const COMPONENTS = [
    ...
    doc.fontSize(8).fillColor('#9E9E99').font('Helvetica')
      .text('Weighted composite · 4 components', MARGIN, y + 5, { width: CONTENT_W - 14, align: 'right' });
    y += 18;
    ...
    // Mirrors app/dashboard.html's QUESTIONS array (same keys/option order,
    // opts[0] = safe/pass, opts[1] = caution, opts[2] = flag).
    const BS_QUESTIONS = [
      { key: 'adult', icon: '\u{1F51E}', label: 'Adult content', opts: ['None', 'Mild', 'Explicit'] },
      { key: 'gambling', icon: '\u{1F3B0}', label: 'Gambling', opts: ['Never', 'Past', 'Active'] },
      { key: 'political', icon: '\u{1F5F3}', label: 'Political', opts: ['None', 'Occasional', 'Frequent'] },
      { key: 'profanity', icon: '\u{1F92C}', label: 'Profanity', opts: ['None', 'Occasional', 'Frequent'] },
      { key: 'family_safe', icon: '\u{1F46A}', label: 'Family safe', opts: ['Yes', 'Mostly', 'No'] },
      { key: 'paid_disclosure', icon: '\u{1F4E2}', label: 'Paid disclosure', opts: ['Always', 'Inconsistent', 'Never'] },
      { key: 'misinformation', icon: '\u{1F6AB}', label: 'Misinformation', opts: ['None', 'Suspected', 'Confirmed'] },
      { key: 'controversy_history', icon: '\u{26A0}', label: 'Controversy', opts: ['None', 'Minor', 'Major'] },
    ];
    const bsMap = {};
    (bsAnswers || []).forEach(a => { bsMap[a.question_key] = a.answer; });

    doc.rect(MARGIN, y, CONTENT_W, 18).fill('#F8F7F4');
    doc.fontSize(8).fillColor('#9E9E99').font('Helvetica-Bold')
      .text('BRAND SAFETY ASSESSMENT', MARGIN + 14, y + 5, { characterSpacing: 0.8 });
    y += 18;

    const bsStartY = y;
    const bsCols = 4;
    const bsGap = 8;
    const bsCellW = (CONTENT_W - bsGap * (bsCols - 1)) / bsCols;
    const bsCellH = 46;
    const bsRows = Math.ceil(BS_QUESTIONS.length / bsCols);
    doc.rect(MARGIN, y, CONTENT_W, bsRows * bsCellH + (bsRows - 1) * bsGap).fill('#F9F8F5');
    BS_QUESTIONS.forEach((q, idx) => {
      const col = idx % bsCols, row = Math.floor(idx / bsCols);
      const cx0 = MARGIN + col * (bsCellW + bsGap);
      const cy0 = y + row * (bsCellH + bsGap);
      doc.rect(cx0, cy0, bsCellW, bsCellH).stroke('#E5E4DF').lineWidth(0.5);
      const answer = bsMap[q.key];
      const optIdx = answer ? q.opts.indexOf(answer) : -1;
      const color = optIdx < 0 ? '#9E9E99' : optIdx === 0 ? '#2D6A0F' : optIdx === 1 ? '#92460A' : '#9E2B22';
      const mark = optIdx < 0 ? '\u2014' : optIdx === 0 ? '\u2713' : optIdx === 1 ? '!' : '\u2715';
      doc.fontSize(8).fillColor('#9E9E99').font('Helvetica')
        .text(q.label, cx0 + 8, cy0 + 8, { width: bsCellW - 16, align: 'center' });
      doc.fontSize(9.5).fillColor(color).font('Helvetica-Bold')
        .text(`${mark} ${answer || 'Not answered'}`, cx0 + 6, cy0 + 26, { width: bsCellW - 12, align: 'center' });
    });
    y += bsRows * bsCellH + (bsRows - 1) * bsGap;
    doc.rect(MARGIN, bsStartY, CONTENT_W, y - bsStartY).stroke('#E5E4DF').lineWidth(0.5);
    y += 14;
    ...
    // ── Recommendation ──────────────────────────────────────────────────────
    const bsComponentValue = compMap['brand_safety']?.value || 0;
    const verdict = deriveVerdict(score, bsComponentValue, verifiedCount);
    // Distinguish "trust/safety are strong, just no verified track record
    // yet" from an actual trust/safety concern -- see matching comment in
    // app/evekit.html's Recommendation card for why these need different
    // headlines instead of one generic "Proceed with Caution".
    const isTrackRecordCaution = verdict === 'caution' && score >= 70 && bsComponentValue >= 70 && verifiedCount < 2;
    const verdictCopy = {
      approve: { text: '\u2713 Recommend to Sponsor', color: '#4ADE80', bg: '#0F1A0C', border: '#2A4020' },
      caution: { text: isTrackRecordCaution ? '\u26A0 Strong Profile \u2014 No Track Record Yet' : '\u26A0 Proceed with Caution', color: '#F0B429', bg: '#1A160C', border: '#4A3A20' },
      avoid:   { text: '\u2715 Not Yet Recommended', color: '#F87171', bg: '#1A0C0C', border: '#4A2020' },
    }[verdict];

    const flaggedCount = Object.entries(bsMap).filter(([k, v]) => {
      const q = BS_QUESTIONS.find(x => x.key === k);
      return q && q.opts.indexOf(v) === 2;
    }).length;
    const recLines = [
      isTrackRecordCaution
        ? `${displayName}'s trust score (${score}/100) and brand safety are in good standing \u2014 the gap is a verified sponsorship track record: no mutually confirmed campaigns yet, so there's no history to reference on delivery or reliability.`
        : `${displayName} currently holds a ${score}/100 trust score (${badgeTier.charAt(0).toUpperCase() + badgeTier.slice(1)} tier) with ${verifiedCount} mutually confirmed campaign${verifiedCount !== 1 ? 's' : ''}.`,
      isTrackRecordCaution ? null : (flaggedCount > 0
        ? `Brand safety review flagged ${flaggedCount} area${flaggedCount !== 1 ? 's' : ''} worth checking before committing.`
        : 'No brand safety flags across the questionnaire.'),
      `Suitable for ${niche} brands${location && location !== '\u2014' ? ` targeting audiences near ${location}` : ''}.`,
    ].filter(Boolean).join(' ');

    const recBoxH = 74;
    doc.roundedRect(MARGIN, y, CONTENT_W, recBoxH, 8).fill(verdictCopy.bg);
    doc.fontSize(15).fillColor(verdictCopy.color).font('Helvetica-Bold')
      .text(verdictCopy.text, MARGIN + 16, y + 14);
    doc.fontSize(9).fillColor('#9E9E99').font('Helvetica')
      .text(recLines, MARGIN + 16, y + 34, { width: CONTENT_W - 32, lineGap: 2 });
    y += recBoxH + 10;

    const riskLevel = bsComponentValue >= 70 ? 'Low' : bsComponentValue >= 50 ? 'Medium' : 'High';
    const recStatW = CONTENT_W / 3 - 6;
    [
      ['Confidence rating', `${confidence}%`],
      ['Risk level', riskLevel],
      ['Campaigns verified', verifiedCount.toString()],
    ].forEach((stat, i) => {
      const sx = MARGIN + i * (recStatW + 9);
      doc.rect(sx, y, recStatW, 40).fill('#F9F8F5').stroke('#E5E4DF').lineWidth(0.5);
      doc.fontSize(7.5).fillColor('#9E9E99').font('Helvetica-Bold')
        .text(stat[0].toUpperCase(), sx + 10, y + 8, { characterSpacing: 0.4 });
      doc.fontSize(14).fillColor('#1A1A18').font('Helvetica-Bold')
        .text(stat[1], sx + 10, y + 20);
    });
    y += 40 + 14;
    ...
    if ((pressMentions || []).length > 0) {
      doc.rect(MARGIN, y, CONTENT_W, 18).fill('#F8F7F4');
      doc.fontSize(8).fillColor('#9E9E99').font('Helvetica-Bold')
        .text('PRESS & FEATURES', MARGIN + 14, y + 5, { characterSpacing: 0.8 });
      doc.fontSize(8).fillColor('#9E9E99').font('Helvetica')
