...
module.exports = async function handleEveKit(req, res) {
  ...
  try {
    ...
    const [
      ...
      admin.from('platform_connections').select('platform, platform_handle, verification_method, follower_count')
    ...
    doc.fontSize(8).fillColor('#5B4FCF').font('Helvetica-Bold')
      .text('EVEKIT · KITSCORE MEDIA KIT', MARGIN, 48, { characterSpacing: 1.5 });
    ...
    if (platList.length === 0) {
      ...
      platList.slice(0, 3).forEach((p, i) => {
        ...
        doc.fontSize(7.5).fillColor(p.verification_method === 'oauth' ? '#2D6A0F' : '#6B6B67').font('Helvetica')
          .text(p.verification_method === 'oauth' ? 'Verified' : 'Linked', px + 12, y + 36);
      ...
      if (rateRangeText) {
        ...
        doc.fontSize(13).fillColor('#5B4FCF').font('Helvetica-Bold')
    ...
    doc.fontSize(7.5).fillColor('#9CA3AF').font('Helvetica')
      .text(
        'EveKit reflects Kitscore data at time of generation. Verified campaigns require mutual confirmation from both creator and sponsor. ' +
    ...
    const filename = `kitscore-evekit-${displayName.replace(/\s+/g, '-').toLowerCase()}.pdf`;
