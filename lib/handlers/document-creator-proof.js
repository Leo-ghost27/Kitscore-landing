// POST /api/documents?type=creator-proof
...
module.exports = async function handleCreatorProofPacket(req, res) {
  ...
  try {
    ...
    // Proof packet PDF export is Pro-only as of July 2026 -- the free
    // watermarked version was removed. Free creators already have the
    // shareable trust profile link (always free) to send sponsors; the
    // clean PDF export is a paid Pro feature now, full stop.
    if (creator.plan !== 'pro') {
      return res.status(402).json({
        error: 'The Proof Packet PDF is a Pro feature. Upgrade to download it, or share your free trust profile link instead.',
        upgradeUrl: '/app/pricing-creator.html',
      });
    }
