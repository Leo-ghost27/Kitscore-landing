...
const { scanPaidDisclosure } = require('../disclosure-scan');
  ...
  try {
    ...
    try {
      if (channel.uploadsPlaylistId) {
        ...
        // Paid-disclosure text scan -- Phase 1 of real FTC/ad-disclosure
        // checking (see 2026-08-01-disclosure-scan.sql for the full design
        // note and why this was held until now). Same non-blocking,
        // flag-only pattern as the brand safety scan above -- it only
        // writes to disclosure_scans, never to score_components, contracts,
        // or escrow. YouTube-only for now: Instagram/TikTok/Twitch don't
        // currently fetch caption/post text via their OAuth libs, so this
        // can't run for those platforms yet.
        if (videos.length > 0) {
          const dscan = await scanPaidDisclosure(videos);
          if (dscan) {
