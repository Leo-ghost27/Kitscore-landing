...
module.exports = async function handleInstagramOauthStart(req, res) {
  ...
  try {
    ...
    // TEMPORARY debug log -- to confirm which Meta app client_id is
    // actually configured in production right now, since the Meta
    // dashboard shows two candidate apps (KitScore / KistScore) and
    // there's no way to read the INSTAGRAM_CLIENT_ID env var value
    // directly. client_id is not a secret (only client_secret is), safe
    // to log. Remove once confirmed.
    console.log('[instagram-oauth-start] authorizeUrl:', authorizeUrl);
