...
module.exports = async function handleCronEscrowStuckNudge(req, res) {
  try {
    ...
    const { data: contracts, error } = await admin.from('contracts')
      .select('id, title, escrow_amount_cents, deliverable_submitted_at, disputed_at, admin_resolved_at, sponsor_id, creator_id, sponsors!inner(company_name), creators!inner(profiles!inner(display_name)), profiles!contracts_sponsor_id_fkey(email)')
    ...
    for (const c of stuck) {
      const sponsorEmail = c.profiles?.email;
