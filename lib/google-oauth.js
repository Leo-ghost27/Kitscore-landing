...
async function fetchYoutubeUploads(accessToken, uploadsPlaylistId, maxResults = 50) {
  ...
  // playlistItems doesn't carry paidProductPlacementDetails -- that field
  // only exists on the videos resource, so a second batched call is needed
  // (videos.list accepts up to 50 comma-separated IDs per call, so this is
  // at most one extra request regardless of how many videos were fetched
  // above). This is YouTube's own record of what the creator declared at
  // upload time via the "paid promotion" checkbox -- see
  // 2026-08-01b-creator-videos-paid-promotion-flag.sql for why this
  // replaced an LLM guess at the same question.
  const ids = videos.map(v => v.videoId).filter(Boolean);
  for (let i = 0; i < ids.length; i += 50) {
    const batch = ids.slice(i, i + 50);
    const params = new URLSearchParams({ part: 'paidProductPlacementDetails', id: batch.join(',') });
    const res = await fetch(`https://www.googleapis.com/youtube/v3/videos?${params.toString()}`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    const data = await res.json();
    if (!res.ok) {
      // Don't fail the whole upload fetch over this -- title/description/
      // publishedAt are already collected and feed content_consistency
      // regardless of whether this secondary field comes through.
      continue;
    }
    for (const item of data.items || []) {
      const match = videos.find(v => v.videoId === item.id);
      if (match) {
        match.hasPaidPromotion = item.paidProductPlacementDetails?.hasPaidProductPlacement ?? null;
      }
    }
  }
