...
function evidenceExpiryEmail({ creatorName, items, evidenceUrl }) {
  const count = items.length;
  const plural = count === 1 ? 'item' : 'items';
  const listText = items.map(i => `- ${i.file_name} (uploaded ${i.uploadedDate})`).join('\n');
  const listHtml = items.map(i => `
    <li style="margin-bottom:6px"><strong>${escapeHtml(i.file_name)}</strong> <span style="color:#9CA3AF">— uploaded ${escapeHtml(i.uploadedDate)}</span></li>`).join('');

  return {
    subject: `${count} evidence ${plural} on Kitscore ${count === 1 ? 'is' : 'are'} 90+ days old`,
    text: `Hi ${creatorName},\n\n${count} evidence ${plural} on your Kitscore profile ${count === 1 ? 'is' : 'are'} now over 90 days old:\n\n${listText}\n\nFresh evidence keeps your trust score meaningful to sponsors. Add updated analytics or campaign proof here:\n${evidenceUrl}\n\nKitscore`,
    html: `
      <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:24px">
        <h2 style="font-size:18px;color:#1A1A1E">Time to refresh your evidence</h2>
        <p style="color:#6B7280;font-size:14px">${count} evidence ${plural} on your Kitscore profile ${count === 1 ? 'is' : 'are'} now over 90 days old:</p>
        <ul style="color:#1A1A1E;font-size:13.5px;padding-left:20px;margin:16px 0">${listHtml}</ul>
        <p style="color:#6B7280;font-size:13px">Fresh evidence keeps your trust score meaningful to sponsors browsing the directory or reviewing your profile.</p>
        <a href="${evidenceUrl}" style="display:inline-block;margin:16px 0;padding:10px 20px;background:#2563EB;color:#fff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:500">Add fresh evidence</a>
        <p style="color:#9CA3AF;font-size:11px;margin-top:24px;border-top:1px solid #E5E7EB;padding-top:16px">Kitscore · kitscore.co · Verified Sponsorship Reputation</p>
      </div>`,
  };
}

module.exports = { sendEmail, teamInviteEmail, sponsorReceiptEmail, reportReadyEmail, refundConfirmationEmail, disputeNotificationEmail, campaignConfirmInviteEmail, evidenceExpiryEmail };
