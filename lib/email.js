...
function contractFlaggedSponsorEmail({ contractTitle, creatorName, adminNote, scanConcerns, canVoid, contractsUrl }) {
  ...
  const nextStep = canVoid
    ...
    : `<p style="color:#6B7280;font-size:13px">This contract is already fully signed, so it can't be voided from your side. Reply to this email or reach Kitscore support to sort out a resolution.</p>`;
  return {
    subject: `Kitscore flagged a problem with "${contractTitle}"`,
    text: `Kitscore reviewed "${contractTitle}" with ${creatorName} and flagged a problem with the terms.\n\n${adminNote}${scanConcerns?.length ? `\n\nFlagged language:\n${scanConcerns.map(c => `"${c}"`).join('\n')}` : ''}\n\n${canVoid ? `To fix this: void this contract and send a new one with corrected terms.` : `This contract is already fully signed and can't be voided -- reach Kitscore support to resolve it.`}\n\n${contractsUrl}\n\nKitscore`,
    html: `
      <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:24px">
