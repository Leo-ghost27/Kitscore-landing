...
function disclosureFlagEmail({ creatorName, rationale, reviewUrl }) {
  return {
    ...
    text: `A disclosure scan flagged ${creatorName} for possible sponsored content without a visible disclosure marker.\n\nRationale: ${rationale}\n\nThis is pattern detection over title/description text only, not a legal compliance determination -- it has NOT changed their live score. Watch the linked video(s) yourself before deciding anything.\n\nReview it here: ${reviewUrl}\n\nKitscore`,
    html: `
      <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:24px">
        <h2 style="font-size:18px;color:#1A1A1E">Possible undisclosed paid content</h2>
        <p style="color:#6B7280;font-size:14px">A scan flagged <strong>${escapeHtml(creatorName)}</strong> for content that reads as sponsored with no visible disclosure marker.</p>
        <div style="background:#FEF3C7;border-radius:6px;padding:14px 16px;margin:16px 0">
          <div style="font-size:12px;color:#92400E;margin-bottom:4px">Scan rationale</div>
          <div style="font-size:14px;color:#1A1A1E">${escapeHtml(rationale)}</div>
        </div>
...
module.exports = { sendEmail, teamInviteEmail, sponsorReceiptEmail, reportReadyEmail, refundConfirmationEmail, disputeNotificationEmail, campaignConfirmInviteEmail, sponsorInviteCreatorEmail, brandSafetyFlagEmail, disclosureFlagEmail, escrowFundedEmail, escrowDeliverableSubmittedEmail, escrowReleasedEmail };
