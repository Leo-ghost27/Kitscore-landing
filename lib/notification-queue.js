...
async function logNotificationFailure(admin, { kind = 'dispute_notification', campaignId, contractId, recipientEmail, error }) {
  try {
    await admin.from('notification_failures').insert({
      kind, campaign_id: campaignId, contract_id: contractId, recipient_email: recipientEmail, error,
    });
  } catch (err) {
